...
> 说明：由于开发周期紧张，团队在后续版本才开始提交至 Git，以下列表按照内部实际完成的迭代顺序记录主要提交内容。
...
| 7        | 安全加固：将用户和管理员密码存储从明文/DES 加密升级为 BCrypt 加盐哈希；引入 spring-security-core 依赖；更新 UserServiceImpl 和 AdminUserServiceImpl 的密码加密与验证逻辑 | `UserServiceImpl.java`, `AdminUserServiceImpl.java`, `pom.xml`         |
